...
@DatasetReader.register("srl-bio")
class SrlReader(DatasetReader):
    """
    This DatasetReader is designed to read in data for semantic role labelling
    in BIO format. It returns a dataset of instances with the
    following fields:
    ...
    def _read(self, file_path: str):
        ...
        # if `file_path` is a URL, redirect to the cache
        srl_data = self.process_srl(file_path)
        logger.info("Reading SRL instances from dataset files at: %s", file_path)
        for (sentence, tags) in srl_data:
            tokens = [Token(t) for t in sentence]
            verb_indicator = [1 if label[-2:] == "-V" else 0 for label in tags]
            yield self.text_to_instance(tokens, verb_indicator, tags)

    def process_srl(self, filename):
        fin = open(filename, 'r')
        data = []
